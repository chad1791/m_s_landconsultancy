...
<html>
  <head>
    ...
    <title>My Courier | Scanners</title>
              ...
              <ul id="settings" class="collapse list-unstyled ">
                ...
                <?php 
                  if($accountType == 1){
                    ...
                    echo '<li class="active"><a href="scanners">Scanners</a></li>';
                  } 
                ?> 
        ...
        <nav class="navbar">
          <div class="container-fluid">
            <div class="navbar-holder d-flex align-items-center justify-content-between">
              <div class="navbar-header"><a id="toggle-btn" href="#" class="menu-btn"><i class="icon-bars"> </i></a><a href="scanners" class="navbar-brand">
                  <div class="brand-text d-none d-md-inline-block"><strong class="text-primary">Scanner Settings</strong></div></a></div>
            ...
            <div class="col-lg-8">
                <div class="card">
                  ...
                  <div class="card-header d-flex align-items-center">
                    <h4>Toggle the checkboxes below to change scanner settings</h4>
                  </div>
                  <div class="card-body">
                    <div class="form-check">
                      <input data-option="1" class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value="option1" <?php if(count($scannerType)>0){if($scannerType[0]['id']=='1'){echo 'checked="checked"';}} ?>>
                      <label class="form-check-label" for="inlineRadio1">Integrated Camera</label>
                    </div>
                    <br/>
                    <div class="form-check">
                      <input data-option="2" class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2" value="option2" <?php if(count($scannerType)>0){if($scannerType[0]['id']=='2'){echo 'checked="checked"';}} ?>>
                      <label class="form-check-label" for="inlineRadio2">Barcode / Qr Code Scanner</label>
                    </div>
                  </div>
                </div> 
            </div>
  ...
  <script>
    $(function(){
      ...
      var radioBtnOne = $('#inlineRadio1');
      var radioBtnTwo = $('#inlineRadio2'); 
      ...
      radioBtnOne
        .on('click', function(){
          ...
          let toggleScanner = confirm('Are you sure you want to change scanning settings?');
          
          if(toggleScanner){

            //call ajax to toggle scanner...
            let id = $(this).data('option');

						var jqxhr = $.ajax({
						  url: baseUrl + 'Admins/toggleScanner',      
						  dataType:'json',
						  type:'post',
						  data: {
							  id:id,
					  ...
					  .done(function( response ){
						  ...
						  if(response != ''){
							  ...
							  toastr.success('Scanning settings were successfully recorded!', 'My Courier Says');
						  }
						  else{
							  toastr.error('Could not change scanning settings at this time. Contact your web administrator!', 'My Courier Says');
						  }
					  });             
          }
          else{
